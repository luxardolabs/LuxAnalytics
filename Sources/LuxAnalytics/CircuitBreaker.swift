...
// MARK: - Integration with LuxAnalytics

extension LuxAnalytics {
    
    /// Circuit breaker for the analytics endpoint
    static let circuitBreaker = CircuitBreaker(
        failureThreshold: 5,
        resetTimeout: 60,
    ...
    static func sendEventsWithCircuitBreaker(
        ...
        // Attempt to send
        let success = await _sendEventsAsync(events, asBatch: asBatch, config: config)
    ...
    public static func getCircuitBreakerStatus() async -> CircuitBreakerMetrics {
        return await circuitBreaker.getMetrics()
    }
